...
f_im = pd.read_csv(f_im_path+'shap_f_importance_cat.csv')
...
Kfold = CVFS(random_seed=random_seed, model_name='Catboost', file_name='catboost')
...
for i in range(len(f_sel)):
    ...
    a = f_sel[:i+1]
    x = X[a]
    acc_df = Kfold.k_fold_roc(model=cat_clf, X=x, y=y, fig_save=False)
    mean_df = acc_df.mean()
    std_df = acc_df.std()
    auc = mean_df['AUC']
...
df.to_csv(save_path+'f_select_auc_cat.csv')
...
f_im = pd.read_csv(f_im_path+'shap_f_importance_sgd.csv')
...
Kfold = CVFS(random_seed=random_seed, model_name='sgd', file_name='sgd')
...
for i in range(len(f_sel)):
    ...
    a = f_sel[:i+1]
    x = X[a]
    acc_df = Kfold.k_fold_roc(model=sgd_clf, X=x, y=y, fig_save=False)
    mean_df = acc_df.mean()
    std_df = acc_df.std()
    auc = mean_df['AUC']
...
df.to_csv(save_path+'f_select_auc_sgd.csv')
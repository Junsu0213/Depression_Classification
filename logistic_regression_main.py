...
f_im = pd.read_csv(f_im_path+'shap_f_importance_lr.csv')
...
Kfold = CVFS(random_seed=random_seed, model_name='lr', file_name='lr')
...
for i in range(len(f_sel)):
    print('select number:', i+1)
    a = f_sel[:i+1]
    x = X[a]
    if  i == 36:
        acc_df = Kfold.k_fold_roc(model=lr_clf, X=x, y=y, fig_save=True)
    else:
        acc_df = Kfold.k_fold_roc(model=lr_clf, X=x, y=y, fig_save=False)
    mean_df = acc_df.mean()
    std_df = acc_df.std()
    auc = mean_df['AUC']
...
df.to_csv(save_path+'f_select_auc_lr.csv')